...
from build_profiles import build_profiles, validate as validate_profiles
...
def validate_event_against_sgt(event: dict, analysis: dict) -> None:
    ...
    if expected_net and actual_net != expected_net:
        raise RuntimeError(
            f"Tournament {event['id']} net winner mismatch: SGT={expected_net!r}, analysis={actual_net!r}"
        )
    if expected_gross and actual_gross != expected_gross:
        raise RuntimeError(
            f"Tournament {event['id']} gross winner mismatch: SGT={expected_gross!r}, analysis={actual_gross!r}"
        )
...
def enrich_metadata(analysis: dict, event: dict) -> dict:
    ...
    tournament["name"] = event.get("tourName") or tournament.get("name")
...
def process_league(*, refresh_all: bool = False) -> dict:
    config = load_config()
    ...
    for event in discovery.get("completed", []):
        ...
        if analysis_path.exists() and not refresh_all:
            ...
            enrich_metadata(analysis, event)
            validate_event_against_sgt(event, analysis)
            write_json(analysis_path, analysis)
            reused.append(event_id)
            print(f"REUSE {event_id}: deterministic analysis validated")
            continue

        print(f"PROCESS {event_id}: {event.get('tourName')} | {event.get('course')} | {event.get('date')}")
        raw = export_tournament(event_id)
        write_json(raw_dir / f"{event_id}.json", raw)
        analysis = enrich_metadata(analyze(raw), event)
        validate_event_against_sgt(event, analysis)
        write_json(analysis_path, analysis)
        processed.append(event_id)

    history = build_profiles(discovery, analyses_dir, config)
    validate_profiles(history)
    history["processing"] = {
        "newlyProcessedEventIds": processed,
        "reusedEventIds": reused,
    }
    write_json(ROOT / "data" / "history.json", history)

    print(
        f"League complete: completed={len(discovery.get('completed', []))} "
        f"eligible={len(history['profileEligibleEvents'])} processed={processed} reused={reused}"
    )
    return history


def main() -> None:
    parser = argparse.ArgumentParser(description="Discover, analyze, and build rolling SGT league history.")
    parser.add_argument("--refresh-all", action="store_true", help="Re-export and re-analyze every completed event.")
    args = parser.parse_args()
    process_league(refresh_all=args.refresh_all)
